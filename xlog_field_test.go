...
func TestFieldCreation(t *testing.T) {
	t.Run("String field", func(t *testing.T) {
		f := xfield.String("key", "value")
		assert.Equal(t, "key", f.Key)
		assert.Equal(t, xfield.StringType, f.Type)
	...
	t.Run("Bools field", func(t *testing.T) {
		f := xfield.Bools("key", []bool{true, false})
		assert.Equal(t, xfield.ArrayType, f.Type)
		assert.Equal(t, int64(0), f.Integer)
	...
	t.Run("Time field", func(t *testing.T) {
		now := time.Now()
		f := xfield.Time("key", now)
		assert.Equal(t, xfield.TimeType, f.Type)
	...
	t.Run("Duration field", func(t *testing.T) {
		d := 5 * time.Second
		f := xfield.Duration("key", d)
		assert.Equal(t, xfield.DurationType, f.Type)
		assert.Equal(t, int64(d), f.Integer)
		assert.Equal(t, "5s", f.FormatValue())
	})

	t.Run("Durations field", func(t *testing.T) {
		f := xfield.Durations("key", []time.Duration{time.Millisecond, time.Second})
		assert.Equal(t, xfield.ArrayType, f.Type)
	...
	t.Run("Err field", func(t *testing.T) {
		...
		assert.Equal(t, xfield.ErrorType, f.Type)
	...
	t.Run("NamedError field", func(t *testing.T) {
		...
		f := xfield.NamedError("custom_key", err)
	...
	t.Run("NamedError field with nil", func(t *testing.T) {
		f := xfield.NamedError("custom_key", nil)
		assert.Equal(t, "custom_key", f.Key)
		assert.Equal(t, xfield.ErrorType, f.Type)
		assert.Equal(t, "", f.String)
		assert.Equal(t, "", f.FormatValue())
	...
	t.Run("Any field", func(t *testing.T) {
		...
		f := xfield.Any("key", obj)
		assert.Equal(t, xfield.AnyType, f.Type)
	...
	t.Run("Binary field", func(t *testing.T) {
		...
		f := xfield.Binary("key", data)
		assert.Equal(t, xfield.BinaryType, f.Type)
	...
	t.Run("Object field", func(t *testing.T) {
		...
		f := xfield.Object("key", obj)
		assert.Equal(t, xfield.ObjectType, f.Type)
		assert.Equal(t, obj, f.Interface)
